...
- **스포티파이 인기도**: 사용자가 안 쓰기로 함 — UI 컬럼과 워크플로 실행에서 제거됨.
  fetch-spotify.mjs 만 dormant로 남아 있음 (스포티파이 정책상 앱 소유 계정에 프리미엄 필요했음).
  재개하려면: 프리미엄 계정으로 시크릿 2개 등록 + 워크플로에 실행 추가 + 템플릿에 컬럼 복원.
